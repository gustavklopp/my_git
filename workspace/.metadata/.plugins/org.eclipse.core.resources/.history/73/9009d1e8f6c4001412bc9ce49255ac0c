...
class _Visitor(nodes.NodeVisitor):
    ...
    def dispatch_visit(self, node):
        ...
        if cls is nodes.document:
            ...
            self.do_strip_text = False
#             print("PARAGRAPH NOT STRIPPED")
            label = RstParagraph(document=self.root)
            if isinstance(self.current, RstEntry):
                label.mx = 10
            label.size_hint_y = None
#             label.padding = ['-5dp', 2]
            label.bind(width=lambda s, w:
                   s.setter('text_size')(s, (w, None)))
            print(label.text_size)
            label.bind(texture_size=label.setter('size'))
            label.bind(size=lambda s, w: print('s :', s, ' and w :', w))
            label.bind(texture_size=self.current.setter('size'))
            self.current.add_widget(label)
            self.push(label)
